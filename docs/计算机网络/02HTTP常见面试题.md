...
3. **并发传输**：针对不同的 HTTP 请求用独一无二的 Stream ID 来区分，接收端可以通过 Stream ID 有序组装成 HTTP 消息，不同 Stream 的帧是可以乱序发送的，因此可以并发不同的 Stream ，也就是 HTTP/2 可以并行交错地发送请求和响应。（TCP层还是一个一个数据包发送，每个包用Stream ID 区分属于哪个请求，接收到包之后，按 Stream ID 将包分类，并组装成完整HTTP报文）（解决队头阻塞）
